...
function handleAnime(req, res) {
    ...
    let sqlQuery = `insert into anime(title, synopsis, type, episodes, score, image_url, start_date,end_date,reated) values ($1,$2,$3,$4,$5,$6,$7,$8,$9)returning *`;
    let values = [
        title,
        synopsis,
        type,
    ...
    client.query(sqlAnime).then(data=>{
        anime_id= data.rows.id;
    })

    let list = `INSERT INTO user_list(user_id, anime_id) values ($1, $2)returning *`;
...
function handleMyList(req,res){
    let sql = `SELECT a.title, a.image_url FROM user_list ul, users u, anime a where ul.user_id= u.id and ul.anime_id= a.id`;
